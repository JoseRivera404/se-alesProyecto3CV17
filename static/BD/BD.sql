...
idUsuario integer, 
idUsuarioDep integer, 
primary key(idUsuario, idUsuarioDep), 
foreign key(idUsuario) references usuarios (idUsuario) 
on delete cascade on update cascade, 
foreign key(idUsuarioDep) references usuarios (idUsuario) on delete cascade on update cascade
);



create table tarea (
idTarea integer auto_increment primary key, 
nombreTarea varchar(100) not null unique, 
valorPuntos bigint not null, 
...
idRecompensa integer auto_increment primary key, 
nombreRecompensa varchar(100) not null unique, 
puntosCuesta bigint not null, 
foto longblob
);
...
foreign key(idRecompensa) references recompensa(idRecompensa) on delete cascade on update cascade, 
foreign key(idUsuario) references usuarios(idUsuario) on delete cascade on update cascade
);